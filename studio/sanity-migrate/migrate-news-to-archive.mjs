...
const hasNewsReference = (value, idMap) => {
...
const newsDocuments = await client.fetch(`*[_type == $sourceType]`, {sourceType})
const idMap = new Map(newsDocuments.map((document) => [document._id, getArchivedId(document._id)]))

console.log(`Found ${newsDocuments.length} ${sourceType} documents.`)

if (!shouldExecute) {
  console.log('Preview only: no documents were changed.')
  console.log('Run the same command with -- --execute to archive every listed news document.')
  process.exit(0)
}
...
await commitInBatches(
  ...
  (batch) => {
    ...
    batch.forEach((document) => {
      const archivedNews = {
        ...rewriteReferences(stripSystemFields(document), idMap),
        _id: idMap.get(document._id),
        _type: targetType,
      }

      transaction.createIfNotExists(archivedNews)
    })
...
await commitInBatches(
  ...
  'Removed original news documents',
)

const [remainingNews, archivedNews] = await Promise.all([
  client.fetch(`count(*[_type == $sourceType])`, {sourceType}),
  client.fetch(`count(*[_type == $targetType])`, {targetType}),
])

console.log(`Finished. Remaining active news: ${remainingNews}. Archived news: ${archivedNews}.`)
